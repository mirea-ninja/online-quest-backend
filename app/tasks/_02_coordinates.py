...
FLAGS = ["ninja{48.785,132.932}", "ninja{48.785183,132.932175}", "ninja{48.78518,132.93217}", "ninja{48.7851,132.9321}"]


def get_flag() -> list[str]:
    return FLAGS
...
def get_task() -> TaskModel:
    return TaskModel(
        ...
        question="Поздравляю, ты преодолел своё первое испытание, но путь предстоит длинный, так что не стоит "
                 "расслабляться. Настоящий мастер должен уметь хорошо ориентироваться на местности, где бы он не "
                 "находился. Тебе предстоит найти правильный ориентир и понять, куда же ты попал!",
        text="",
