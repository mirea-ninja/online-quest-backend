...
def get_task() -> TaskModel:
    return TaskModel(
        ...
        question="Прекрасно, ты достойно прошел испытание. Мой ученик, запомни, без прошлого не может быть и будущего. Мастеру просто необходимо знать историю своей деревни IIT. С 2015 года панды заботятся об экологии, устраивая каждый год торжественную церемонию сбора бамбука для переработки, а также выкладывая результаты сбора. Именно на этом материале мастера увековечивали свои знания",
        text="Введите ответ",
        media_links=[],
    )
